...
### Question to surface at PR-open time (Phase 0.8)

- **YouTube now included in `postAllPlatforms` "Post all" set.** Pre-Phase-0.8, a filter bug at `phase-5-post.tsx:103` silently excluded YouTube from "Post all" — Yonah had to post YouTube separately via the YouTube card. Phase 0.8 fix (commit `79ad01f`) closes that filter bug. Going forward, "Post all" will fan out to YouTube too (YT goes via YouTube Data API directly, not Buffer — so this does NOT affect Buffer call frequency). Confirm with Yonah whether this is the desired workflow before merging. If he expects to post YT separately (e.g. because of the thumbnail picker), either revert `79ad01f` OR add a thumbnail preview to the YouTube row in the BottomSheet so he sees what cover frame will ship.

### Discovered 2026-06-03 during Phase 0.6 review

- [ ] **Harden `auto-post.ts` to read `videos.youtube_thumbnail_url` as 2nd fallback** *[15min, code]*. Phase 0.6 (commit `d1fc3aa`) wired the operator-picked thumb through the dashboard "Post all" prop chain, but autopilot webhook + `scheduleAll` still pass nothing → `auto-post.ts` falls back to `videos.thumb_path` (auto-extracted), silently dropping the operator's pick. Fix: add `youtube_thumbnail_url` to the SELECT in `auto-post.ts:~121` and change the fallback chain at line 412 to `args.youtubeThumbnailUrl ?? video.youtube_thumbnail_url ?? thumbUrl`. ~3 lines. **Do NOT bundle with Phase 0.6** — any change to `auto-post.ts` touches the Buffer fanout code path and we're holding off on those changes until after Yonah's posting window closes. Ship as standalone item Phase 0.6-bis after Yonah's done.
